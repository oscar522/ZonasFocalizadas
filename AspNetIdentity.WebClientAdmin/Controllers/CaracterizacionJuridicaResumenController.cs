...
namespace AspNetIdentity.WebClientAdmin.Controllers
{
    ...
    public class CaracterizacionJuridicaResumenController : BaseController
    {
        ...
        public async Task<ActionResult> ResumenListaDetalle(int IdDeptoIn, int IdCiudadIn, string GrupoIn)
        {
            ...
            if (GrupoIn == "INSPECCION OCULAR")
            {
                var ResultFindINSPECCION_OCULAR = processModel.Where(x => x.IdMunicipio == IdCiudadIn && x.IdDepto == IdDeptoIn.ToString()).ToList();
                var ResultINSPECCION_OCULAR = ResultFindINSPECCION_OCULAR.Where(x => x.Tipo == "INSPECCION OCULAR").ToList();

                List<CaracterizacionJuridicaResumenModel> ResultResoluciones = ResultINSPECCION_OCULAR
                .GroupBy(c => c.INSPECCION_OCULAR_CONCEPTO_57)
                .Select(c => new CaracterizacionJuridicaResumenModel
                {
                    INSPECCION_OCULAR_VISIBLE = c.Select(v => v.INSPECCION_OCULAR_CONCEPTO_57).FirstOrDefault(),// resolucion,
                    NombreMuni = c.Select(v => v.Tipo).FirstOrDefault(),// Tipo de recurso,
                    SOLICITUD_VISIBLE = c.Select(v => v.RECURSO_SOLICITANTE_DECISION_SOBRE_LA_ADJUDICAICON_92).FirstOrDefault(),// resolucion
                    Gestion = c.Count()
                }).ToList();
               ...
               .Select(c => new CaracterizacionJuridicaResumenModel
               {
                   ...
                   //Sin firma 
                   RECURSO_SOLICITANTE_ACTO_89 = ResultINSPECCION_OCULAR.Join(ResultINSPECCION_OCULAR, l => l.Id, m => m.Id, (l, m) => new
                   { l, m }).Where(x => x.l.INSPECCION_OCULAR_FIRMA_55 == false && x.l.Tipo == c.NombreMuni && x.l.INSPECCION_OCULAR_CONCEPTO_57 == c.INSPECCION_OCULAR_VISIBLE)
                            .ToList()
                            .Count(),

                   // opocisiones 
                   RECURSO_SOLICITANTE_VISIBLE = ResultINSPECCION_OCULAR.Join(ResultINSPECCION_OCULAR, l => l.Id, m => m.Id, (l, m) => new
                   { l, m }).Where(x => x.l.INSPECCION_OCULAR_OPOSICIONES_56 == true && x.l.Tipo == c.NombreMuni && x.l.INSPECCION_OCULAR_CONCEPTO_57 == c.INSPECCION_OCULAR_VISIBLE)
                            .ToList()
                            .Count(),

                   // con firma 
                   IdMunicipio = ResultINSPECCION_OCULAR.Join(ResultINSPECCION_OCULAR, l => l.Id, m => m.Id, (l, m) => new
                   { l, m }).Where(x => x.l.INSPECCION_OCULAR_FIRMA_55 == true && x.l.Tipo == c.NombreMuni && x.l.INSPECCION_OCULAR_CONCEPTO_57 == c.INSPECCION_OCULAR_VISIBLE)
                            .ToList()
                            .Count(),
                   Gestion = c.Gestion,
